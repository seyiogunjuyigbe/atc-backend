...
module.exports = {
    async createRecipient(bankaccount) {
        let { currency, type, sort_code, accountHolderName } = bankaccount;
        try {
            let recipient = await TwClient.createRecipientAccount({
                currency, type, sort_code, profile: TRANSFERWISE_PROFILE_ID, ownedByCustomer: false, accountHolderName, details: bankaccount
            })
            return recipient
        } catch (err) {
            return err
    ...
    async deleteRecipientAccount(accountId) {
        try {
            let recipient = await TwClient.deleteRecipientAccount({ accountId })
            return recipient
        } catch (err) {
            return err
        }
    ...
    async initateTransfer(targetAccount, quote, ref) {
        try {
            let transfer = await TwClient.createTransfer({
    ...
    async completeTransfer(transferId) {
        try {
            let payout = await TwClient.fundTransfer({ transferId, type: "BALANCE" });
            console.log(payout)
            return payout
        } catch (err) {
            return err.message
