...
module.exports = {
  async createActivity(req, res) {
    ...
    try {
      ...
      const city = await State.findById(cityId);
      const existingPack = await Activity.findOne({
        title,
        vendor: req.user._id,
      });
      if (existingPack)
        return error(
          res,
          409,
          `Duplicate name: Activity "${title}" already exists`
        );
  ...
  async updateActivity(req, res) {
    ...
    try {
      const country = await Country.findById(countryId);
      const city = await State.findById(cityId);
      const thisActivity = await Activity.findById(req.params.activityId);
      if (!thisActivity) return error(res, 404, 'Activity not found');
      if (String(thisActivity.vendor) !== req.user.id)
  ...
  async fetchAllActivities(req, res) {
    try {
      const activities = await Queryservice.find(Activity, req);
